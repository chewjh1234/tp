...
While we know that you cannot wait to try out some exciting commands, let us learn about commands in the next section, [Command Format](#command-format).
...
| `[ ]…​` | Optional field with no limit on the number of entries | `[t/TAGS]…​` | The student can have a variable number of available tags |
...
If you would like more practice, here are some commands you can try:
...
**Great job!** You can proceed to learn about the exciting [features](#features) we have designed for you. Enjoy!

<div markdown="block" class="alert alert-info">

**:information_source: More notes about the command format:**<br>

* Extraneous characters or words for commands made up of a command word only (such as `help`, `list`, `exit` and `clear`) will be ignored.<br>
